...
log_dir = "/tmp/image_helper/logs"
os.makedirs(log_dir, exist_ok=True)
logging.basicConfig(filename=os.path.join(log_dir, 'running_logs.log'), level=logging.INFO, format=logging_str,
                    filemode="a")

def save_image(image: FileStorage, folder: str = None, name: str = None) -> str:
    return UPLOAD_FOLDER + f"/{photos.save(image, folder, name)}"


def get_path(filename: str = None, folder: str = None) -> str:
    return photos.path(filename, folder)
...
def find_image_any_format(filename: str, folder: str) -> Union[str, None]:
    ...
    for _format in IMAGES:  # look for existing avatar and delete it
        ...
        avatar_path = photos.path(filename=avatar, folder=folder)
